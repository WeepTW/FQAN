#!/usr/bin/env python3
"""Audit complete Mistral base m/d v5 generation and six-field evaluation."""
...
POLICY = "native-user-message-target-last-answer-boundary-v3"
PROMPT_POLICY_VERSION = "narrative2-binding-prompt-v4+mistral-native-chat-template-v2"
...
def audit(generation_root: Path, events_path: Path, output: Path) -> dict[str, Any]:
    ...
    snapshot_path = generation_root / "generation_config.snapshot.json"
    snapshot = read_json(snapshot_path)
    contract = snapshot.get("mistralDirectChatTemplate") or {}
    if (
        contract.get("policy") != POLICY
        or contract.get("baseModel") != MODEL
        or contract.get("chatTemplateSha256") != CHAT_TEMPLATE_SHA256
        or (snapshot.get("promptBuilder") or {}).get("promptPolicyVersion") != PROMPT_POLICY_VERSION
    ):
        raise AuditError("v5 frozen prompt policy/model identity mismatch")
    repo_root = Path(__file__).resolve().parents[1]
    contract_files: list[Path] = []
    for name in ("wrapper", "wrapperBase", "inference", "inferenceBase"):
        spec = contract.get(name) or {}
        root_name = str(spec.get("root") or "")
        relative = str(spec.get("path") or "")
        if root_name == "repo":
            path = repo_root / relative
        elif root_name == "dist":
            path = repo_root / "dist" / relative
        else:
            raise AuditError(f"unsupported v5 implementation root: {root_name}")
        if not path.is_file() or sha256_file(path) != spec.get("sha256"):
            raise AuditError(f"v5 implementation identity mismatch: {name} {path}")
        contract_files.append(path)
    compatibility_path = generation_root / "compatibility_fingerprint.json"
    compatibility = read_json(compatibility_path)
    compatibility_material = compatibility.get("material") or {}
    if (
        compatibility_material.get("generationConfigSha256") != sha256_file(snapshot_path)
        or compatibility_material.get("promptPolicyVersion") != PROMPT_POLICY_VERSION
    ):
        raise AuditError("v5 config/compatibility identity mismatch")
    ...
    prompt_echo_rows = 0
    inventory_files: list[Path] = [events_path, snapshot_path, compatibility_path, *contract_files]
    ...
    if next(iter(fingerprints)) != compatibility.get("sha256"):
        raise AuditError("manifest/root compatibility fingerprint mismatch")
    if prompt_echo_rows:
