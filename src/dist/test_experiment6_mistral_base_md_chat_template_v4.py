#!/usr/bin/env python3
"""Regression tests for Mistral target-last chat prompt ordering."""
...
MODULE_PATH = (
    REPO / ".external/FINDER/Retriever Codes/Mistral/mistral_direct_binding_chat_inference_v3.py"
)
SPEC = importlib.util.spec_from_file_location("mistral_target_last_test", MODULE_PATH)
...
class TargetLastPromptTests(unittest.TestCase):
    def test_moves_examples_before_target_and_preserves_sections(self) -> None:
        ...
        self.assertEqual(
            sorted(PROMPT.rstrip("\n").split("\n\n")),
            sorted(transformed.rstrip("\n").split("\n\n")),
        )
