#!/usr/bin/env python3
"""Record an audited Mistral base m/d v5 completion in docs/log."""
...
KIND = "experiment6_mistral_base_md_chat_template_v5_complete"
...
def markdown(audit: Mapping[str, Any], json_relative: str) -> str:
    lines = [
        "# Experiment 6 Mistral base_m/base_d v5 completion",
        "",
...
def record(audit_path: Path, log_root: Path, index_path: Path, workspace: Path) -> dict[str, Any]:
    ...
    if audit.get("status") != "complete" or audit.get("protocol") != "experiment6-mistral-base-md-v5-completion-audit-v1":
    ...
    base = f"{stamp}_experiment6_mistral_base_md_chat_template_v5_complete"
