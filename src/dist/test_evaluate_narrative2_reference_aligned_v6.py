...
import evaluate_narrative2_reference_aligned_v6 as v6


CONFIG = json.loads(
    (DIST.parent / "config" / "experiment6_narrative2_evaluation_v6.json").read_text(
        encoding="utf-8"
    )
)
...
def prediction(result, format_valid=True):
    return {
        "source": "S1",
        "result": result,
